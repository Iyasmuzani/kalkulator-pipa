  ...
  <div style="background:rgba(0,255,157,.04);border:1px solid rgba(0,255,157,.15);border-radius:10px;padding:12px;margin-top:4px">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
      ...
      <div style="display:flex;gap:6px">
        <button onclick="exportFusionPDF()" style="display:inline-flex;align-items:center;gap:4px;padding:4px 10px;background:rgba(0,229,255,0.12);border:1px solid rgba(0,229,255,0.3);color:#00e5ff;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer;transition:all 0.2s" onmouseover="this.style.background='rgba(0,229,255,0.25)'" onmouseout="this.style.background='rgba(0,229,255,0.12)'">
...
function exportFusionPDF() {
  ...
  fusionBatchData.forEach(function(item, idx) {
    ...
    html += '<td class="fusion-val" style="color:#00e5ff">' + item.GP_fuse.toFixed(1) + '</td>';
    html += '<td class="fusion-val">' + heatMin + 'm ' + heatSec + 's</td>';
    html += '<td class="fusion-val">' + coolMin + 'm ' + coolSec + 's</td>';
    html += '<td class="fusion-val">' + item.Thp + (item.ThpTol ? ' ' + item.ThpTol : '') + '</td>';
    html += '</tr>';
  });
  html += '</table></div>';
  ...
  html += '<strong style="color:#6dd5ed">Parameter Mesin:</strong> Cylinder Area (Ac) = ' + firstItem.Ac.toLocaleString() + ' mm² &nbsp;|&nbsp; Drag Pressure (DP) = ' + firstItem.DP + ' bar &nbsp;|&nbsp; Suhu Ambien = ' + firstItem.temp + '°C';
  html += '</div>';
  html += '</div>';

  // Detail section for each pipe on screen
  fusionBatchData.forEach(function(item, idx) {
    var heatMin = Math.floor(item.heatSoakTime / 60);
    var heatSec = item.heatSoakTime % 60;
    var coolMin = Math.floor(item.coolingTime / 60);
